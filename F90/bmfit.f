...
c
      ...
      COMMON/FIT/ IFT,NP,V(npd),E(npd)
      DIMENSION B(npd),EFIT(npd)
      EXTERNAL FE
      open(11,file='POSCAR')
      read(11,*) temp1
      read(11,*) acel
      ...
      enddo
      vol=aa(1,1)*aa(2,2)*aa(3,3) + aa(1,2)*aa(2,3)*aa(3,1)
  ...
  101 FORMAT(10A8)
   ...
   23 FORMAT(/,' INITIAL GUESS   =',3F10.5,/)
c     PRINT 25,DEL,NDIV
   25 FORMAT(5X,F10.5,5X,I5)
      READ(5,*) NP,IFT,NPLOT
c     PRINT 11,NP,IFT
      if ( np .gt. npd ) stop ' npd too small '
...
c      WRITE(2,1) NP
  ...
  100 CONTINUE
   ...
   20 CONTINUE
    ...
    4 FORMAT(1H ,8F15.8)
      PRINT 27
   27 format('        volume      energy-input   energy-output 
     &    error     ')
      DO 40 I=1,NP
      DE=EFIT(I)-E(I)
      PRINT 4,B(I),E(I),EFIT(I),DE
   40 CONTINUE
...
c
      BM=BM/scal
c     PRINT 26,PAR1(1),PAR1(2),PAR1(3),BM,EMIN
      PRINT 26,PAR1(1),PAR1(2),PAR1(3)
 26   format(/,' FINAL PARAMETER =',3f10.5)
      EB=-PAR1(1)
      AM=(1.5*PAR1(3)/TPI)**(1./3.)
      XL=SQRT(0.5*EB/PAR1(2))/(2.*TPI*AM*AM)
      BP=1.+(AM+AM)/(3.*XL)
c     PRINT 5,EB,AM,XL,BP
      alat = (par1(3)/vol)**(1.0/3.0)
      write(6,1997) alat,par1(3),bm,emin
1997  format(/,' lattice const =',f10.5/' volume        =',f10.5/
     &   ' bulk modulus  =',f10.5/' rms-error     =',f10.5)
